...
document.querySelector("#message-form").addEventListener("submit", (e) => {
  ...
  socket.emit("sendMessage", { message, senderId, senderName, createdAt });
  e.target.elements.message.value = "";
});

socket.on("newMessage", ({ message, senderId, senderName, createdAt }) => {
...
socket.on("newUserJoined", (username) => {
  ...
  messageDiv.innerHTML += `${username} has joined.`;
});
...
socket.on("userLeft", (username) => {
  const messageDiv = document.querySelector("#messages");
  messageDiv.innerHTML += `${username} has left.`;
});
